...
class Sanitizer
{
    ...
    public static function apply_rule($value, $type, array $data = [])
    {
        ...
        if (is_null($type)) {
            throw new InvalidArgumentException(esc_html__('The rule type cannot be null.', 'elemacy'));
        }
        ...
        switch ($type) {
            ...
            default:
                if (is_callable($type)) {
                    ...
                    throw new InvalidArgumentException(esc_html__('Provide valid rule type.', 'elemacy'));
                }
                break;
        }
