...
class MaxRule extends BaseRule
{
    ...
    {
        if ($this->is_string_value()) {
            return sprintf(__('The %s field must be less than or equal to %s characters.', 'droip'), $this->key, $this->rule_value);
        } elseif ($this->is_array_value()) {
            return sprintf(__('The %s field must be less than or equal to %s items.', 'droip'), $this->key, $this->rule_value);
        }

        return sprintf(__('The %s field must be less than or equal %s.', 'droip'), $this->key, $this->rule_value);
    }
