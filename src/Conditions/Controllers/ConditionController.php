...
namespace Elemacy\Conditions\Controllers;

defined('ABSPATH') || exit;

use Elemacy\Core\Http\Request;
use Elemacy\Core\Http\Response;
use Elemacy\Conditions\Resources\ConditionGroupResource;
use Elemacy\Conditions\ConditionManager;
use Elemacy\Conditions\ConditionSearchService;

class ConditionController
{
    public function types(Request $request): Response
    {
        $context = (string) $request->get_string('context', (string) $request->get_string('template_type', ''));
        $result  = [];

        foreach (ConditionManager::instance()->get_grouped() as $type => $conditions) {
            $applicable = array_filter(
                $conditions,
                static fn ($condition): bool => $condition->applies_to($context)
            );
