...
export function TriggerRow({ entry, types, onChange, onRemove }: TriggerRowProps) {
            ...
            <div className="flex items-center gap-2">
                <Select value={entry.type || undefined} onValueChange={handleTypeChange}>
                    <SelectTrigger className="flex-1 min-w-0">
                        <SelectValue placeholder={__('Select trigger…', 'elemacy')} />
                    </SelectTrigger>
                    <SelectContent>
                        {types.map((type) => {
                ...
                <Button
                    ...
                    size="sm"
                    onClick={onRemove}
                    aria-label={__('Remove trigger', 'elemacy')}
